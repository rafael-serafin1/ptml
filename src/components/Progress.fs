...
module Progress =   
    ...
    let settingFrames(tp: ProgressType) = 
        match tp with
        | Blocks -> 
            ()
        | Dots ->
    ...
    let framefy(w: int, h: int, tp: ProgressType, value: int, max: int, show: string) =
        let show: bool = 
            match show with
            | "true" -> true
            | "false" -> false
        ...
        if show then
            let maxed: string = " " + (string) (Utils.regrade3(max, value)) + "%"
